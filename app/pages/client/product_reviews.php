...
if ($productId <= 0) {
    ...
    try {
        $stmt = db()->prepare(
            'SELECT p.id, p.name, p.base_price, p.shop_id, s.name AS shop_name
             FROM products p
...
if ($product && get_table_columns(db(), 'reviews')) {
    try {
...
?>
<h1 class="h4 mb-3">Product Reviews</h1>
...
<?php if ($product): ?>
    <div class="card shadow-sm mb-4">
        <div class="card-body">
            <div class="d-flex justify-content-between align-items-start flex-wrap gap-2">
                <div>
                    <h2 class="h5 mb-1"><?= htmlspecialchars($product['name'], ENT_QUOTES, 'UTF-8') ?></h2>
                    <div class="text-muted small">Shop: <?= htmlspecialchars($product['shop_name'] ?? 'Shop', ENT_QUOTES, 'UTF-8') ?></div>
                </div>
                <div class="text-end">
                    <div class="fw-semibold">₱<?= number_format((float) ($product['base_price'] ?? 0), 2) ?></div>
    ...
    <?php else: ?>
        <div class="card shadow-sm">
            <div class="card-body">
                <p class="text-muted mb-0">No reviews yet. Completed orders can leave feedback from their order details.</p>
            </div>
        </div>
    <?php endif; ?>

    <div class="mt-3">
        <a class="btn btn-outline-secondary" href="/client/product_view?id=<?= (int) $product['id'] ?>">Back to product</a>
    </div>
<?php else: ?>
    <div class="mt-3">
