...
if (!$errors) {
    try {
        $filters = 'o.shop_id = :shop_id AND o.status = :completed AND p.status = :verified';
        $params = [
            'shop_id' => $shop['id'],
            'completed' => 'completed',
            'verified' => 'verified',
                    ...
                    <tbody>
                        ...
                        <?php foreach ($earningRows as $row): ?>
                            <tr>
                                ...
                                <td><?= htmlspecialchars($row['payment_created_at'] ?? $row['order_created_at'] ?? '—', ENT_QUOTES, 'UTF-8') ?></td>
                            </tr>
                        <?php endforeach; ?>
                    </tbody>
                </table>
